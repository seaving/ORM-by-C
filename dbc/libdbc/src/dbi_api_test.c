...
int dbi_api_test()
{
	...
	if (results == NULL)
	{
		LOG_DEBUG_TRACE("result was null!\n");
		dbi_error_debug(obj);
		goto exit;
	}
	LOG_DEBUG_TRACE("result rows count: %llu\n", dbi_result_get_rows_count(results));
	...
	{
		LOG_DEBUG_TRACE("dbi_result_row_add error!\n");
		dbi_error_debug(obj);
	}
	LOG_DEBUG_TRACE("result rows count: %llu\n", dbi_result_get_rows_count(results));
	LOG_DEBUG_TRACE("-------------------------------\n");
	...
	dbi_result_row_foreach(results)
	{
		...
		LOG_DEBUG_TRACE("dbi_result_field_get_length_by_fieldname: %u\n", n);
		n = dbi_result_field_get_length_by_idx(results, 4);
		LOG_DEBUG_TRACE("dbi_result_field_get_length_by_idx: %u\n", n);
		n = dbi_result_field_get_idx(results, "ADDRESS");
		LOG_DEBUG_TRACE("dbi_result_field_get_idx: %u\n", n);
		for (i = 1; i <= dbi_result_field_get_count(results); i ++)
		{
			fieldname = dbi_result_field_get_name(results, i);
			LOG_DEBUG_TRACE("field name: %s\n", fieldname);
			LOG_DEBUG_TRACE("field type by name: %u\n", 
					dbi_result_field_get_type_by_fieldname(results, fieldname));
			LOG_DEBUG_TRACE("field type by idx: %u\n", 
